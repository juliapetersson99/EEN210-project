...
labels = ["falling", "walking", "running", "sitting", "standing", "lying", "recover"]
label_colors = {
    'falling': '#e41a1c',    # Red
    'walking': '#377eb8',    # Blue
...
def plot_with_labels(data_frame, plt_name):
    ...
    for _, row in intervals.iterrows():
        ...
        for sub_ax in ax:
            sub_ax.axvspan(row['start'], row['end'], 
                            color=label_colors[row['label']], 
                            alpha=0.2,  #Visibiulity
                            zorder=0)  #order of plots in the z-direction
    ...
    patches = [
        ...
        for label in label_colors if label in present_labels ]
